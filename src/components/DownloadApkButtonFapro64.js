...
const DownloadApkButtonFapro64 = () => {
  const [isLoading, setIsLoading] = useState(false);

  const handleDownload = () => {
    setIsLoading(true);

      const apkUrl = 'https://fapro-plus.web.app/faproplus2.apk'; // Reemplaza con la URL real de tu APK
  ...
  return (
    <DownloadButton onClick={handleDownload} disabled={isLoading}>
      {isLoading ? <Spinner /> : 'Descargar APK (opcion 2)'}
      {isLoading && 'Descargando...'}
    </DownloadButton>
  );
};

export default DownloadApkButtonFapro64;
